...
.welcome-screen{display:flex;flex-direction:column;align-items:center;gap:14px;padding:16px;width:100%;max-width:520px;margin:0 auto;padding-top:12px}
@media(max-width:700px){
  .welcome-screen{gap:8px;padding:8px}
  .welcome-screen .sugg{font-size:11px;padding:7px 9px}
  .welcome-screen h-logo{width:36px!important;height:36px!important;font-size:18px!important}
  ...
  const sendMessage = useCallback(async (text: string) => {
    ...
    if (allMatches.length > 1) {
      setMessages(prev => prev.map(m => m.id === aiId ? {
        ...
        content: `__DISAMBIGUATE__${JSON.stringify(allMatches.map(q => q.pergunta))}`
      } : m))
      setStreaming(false)
      return
    }
    ...
    <>
      ...
      <div className="root">
        ...
        <div style={{ flex:1, display:'flex', flexDirection:'column', minWidth:0 }}>
          ...
          {view==='chat' && (
            <>
              <div className="chat-area" style={{ justifyContent: messages.length === 0 ? (window.innerWidth > 700 ? 'center' : 'flex-start') : 'flex-start' }}>
                {messages.length===0 ? (
                  ...
                  if (msg.content.startsWith('__DISAMBIGUATE__')) {
                    const options: string[] = JSON.parse(msg.content.replace('__DISAMBIGUATE__',''))
                    return (
                      <div key={msg.id} style={{ background:'#1a1730', border:'1px solid #3d3580', borderRadius:10, padding:'12px 14px', maxWidth:'85%' }}>
                        ...
                        {options.map((s,i) => (
                          <div key={i} onClick={() => sendMessage(s)}
                            ...
                            <span style={{ color:'#7c6af7', fontWeight:500 }}>{i+1}.</span> {s}
                          </div>
                        ))}
                      </div>
                    )
