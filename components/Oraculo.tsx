...
.root{font-family:'Sora',sans-serif;background:#0d0d0f;color:#e8e8f0;height:100vh;display:flex;overflow:hidden;position:relative}
...
.welcome-screen{display:flex;flex-direction:column;align-items:center;gap:14px;padding:16px;width:100%;max-width:520px;margin:0 auto}
@media(max-width:700px){
  .welcome-screen{gap:10px;padding:10px}
  .welcome-screen .sugg{font-size:11px;padding:8px 10px}
}
    ...
    <>
      ...
      <div className="root">
        ...
        <div style={{ flex:1, display:'flex', flexDirection:'column', minWidth:0 }}>
          ...
          {view==='chat' && (
            <>
              <div className="chat-area" style={{ justifyContent: messages.length === 0 ? 'center' : 'flex-start' }}>
                {messages.length===0 ? (
                  <div className="welcome-screen">
                    ...
                    <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:7, width:'100%' }}>
                      {SUGGESTIONS_DEFAULT.map((s,i)=><button key={i} className="sugg" onClick={()=>sendMessage(s)}>{s}</button>)}
                    </div>
                  </div>
                ) : messages.map(msg => {
                ...
                <div key={qa.id} className="qa-card">
                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:8 }}>
                    ...
                    <div style={{ display:'flex', gap:6, flexShrink:0, marginLeft:12 }}>
                      ...
                      <button onClick={()=>setQaBase(prev=>prev.filter(q=>q.id!==qa.id))} style={{ padding:'2px 8px', background:'transparent', border:'1px solid #3a0000', borderRadius:4, color:'#f87171', fontSize:10, cursor:'pointer', fontFamily:"'Sora',sans-serif" }}>Remover</button>
                    </div>
                  </div>
