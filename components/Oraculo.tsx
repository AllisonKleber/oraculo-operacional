...
export default function Oraculo() {
  ...
  const sendMessage = useCallback(async (text: string) => {
    ...
    // 1. Verificar múltiplos Q&As manuais com a mesma palavra-chave
    ...
    // 2. Q&A manual tem prioridade
    const manualQA = searchQA(text, qaBase)
    let response: string
    let figuras: string[] = []
    let imagens: string[] = []
    let found: boolean
    ...
    if (manualQA) {
      response = manualQA.resposta
      figuras = manualQA.figuras
      imagens = manualQA.imagens || []
      found = true
    } else {
      // 3. Fallback: Claude API com conhecimento extraído pelo Docling
      try {
        ...
        response = data.answer || 'Não encontrei resposta para essa consulta na base normativa atual.'
        found = data.found !== false
      } catch {
        response = 'Erro ao consultar a base de conhecimento. Tente novamente.'
        found = false
      }
    }
  ...
  return (
    <>
      ...
      <div className="root">
        ...
        {/* Lightbox de imagem */}
        {lightboxSrc && (
          <div onClick={() => setLightboxSrc(null)} style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.93)', zIndex:1000, display:'flex', alignItems:'center', justifyContent:'center', cursor:'zoom-out' }}>
            <img src={lightboxSrc} alt="visualização" style={{ maxWidth:'92vw', maxHeight:'92vh', borderRadius:10, border:'1px solid #2a2a32', objectFit:'contain' }} onClick={e => e.stopPropagation()} />
            <button onClick={() => setLightboxSrc(null)} style={{ position:'absolute', top:16, right:16, width:36, height:36, borderRadius:'50%', background:'#18181c', border:'1px solid #2a2a32', color:'#e8e8f0', fontSize:18, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center' }}>✕</button>
          </div>
        )}
                      ...
                      <div style={{ maxWidth:'82%', display:'flex', flexDirection:'column', gap:4 }}>
                        ...
                        {msg.role==='ai' && !msg.typing && msg.figuras && msg.figuras.map(figNum => FIGURAS[figNum] && (
                          <div key={figNum} className="fig-box">
                            ...
                            <img src={FIGURAS[figNum].src} alt={FIGURAS[figNum].titulo} className="fig-img" style={{ cursor:'zoom-in' }} onClick={() => setLightboxSrc(FIGURAS[figNum].src)} />
                          </div>
                        ))}
                        ...
                        {msg.role==='ai' && !msg.typing && msg.imagens && msg.imagens.map((src,i) => (
                          <div key={i} className="fig-box">
                            ...
                            <img src={src} alt={`ref-${i}`} className="fig-img" style={{ cursor:'zoom-in' }} onClick={() => setLightboxSrc(src)} />
                          </div>
                        ))}
...
function LoginScreen({ onLogin }: { onLogin: (u: User) => void }) {
  ...
  const handle = () => {
    ...
    if (isAdmin && senha !== '@AnaNanda10') return setErr('Senha de administrador incorreta.')
    onLogin({ name:name.trim(), matricula:'', tipo, role:isAdmin?'admin':'usuario' })
  }
    ...
    <>
      ...
      <div style={{ minHeight:'100vh', background:'#0d0d0f', display:'flex', alignItems:'center', justifyContent:'center', padding:16 }}>
        <div style={{ background:'#18181c', border:'1px solid #2a2a32', borderRadius:16, padding:32, width:'100%', maxWidth:400 }}>
          ...
          <button className="login-btn" onClick={handle} disabled={!name}>Entrar no Oráculo</button>
